...
@section('title', 'Movement Details - Gatekeeper')
        ...
        <div>
            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0">
                    <li class="breadcrumb-item"><a href="{{ route('gatekeeper.movements.index') }}" class="text-decoration-none">Logs</a></li>
                    <li class="breadcrumb-item active" aria-current="page">#{{ $movement->id }}</li>
                </ol>
            </nav>
        </div>
        <div class="d-flex gap-2">
            <a href="{{ route('gatekeeper.movements.index') }}" class="btn btn-outline-secondary btn-sm px-4 rounded-pill fw-bold">
            ...
            <div class="card border-0 shadow-sm rounded-4 mb-4 overflow-hidden">
                ...
                <div class="card-body p-4">
                    <div class="row align-items-center">
                        <div class="col-md-7">
                            <h3 class="fw-bold text-dark mb-1">{{ $movement->product_name }}</h3>
                            <p class="text-muted mb-4">Reference: {{ $movement->authorization_reference ?: 'N/A' }}</p>
                            
                            <div class="d-flex gap-4 mb-2">
                                <div>
                                    <small class="text-muted d-block text-uppercase fw-bold ls-1 mb-1">Quantity</small>
                                    <span class="fs-4 fw-bold text-dark">{{ $movement->quantity }}</span>
                                </div>
                                <div>
                                    <small class="text-muted d-block text-uppercase fw-bold ls-1 mb-1">Unit Price</small>
                                    <span class="fs-4 fw-bold text-dark">{{ $movement->unit_price ? number_format($movement->unit_price, 2) . ' TZS' : '-' }}</span>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-5">
                            <div class="p-3 bg-light rounded-4 border text-center">
                                ...
                                <div class="fw-bold text-dark fs-5">{{ $movement->movement_date->format('M d, Y') }}</div>
                                <div class="text-primary fw-medium">{{ $movement->movement_date->format('h:i A') }}</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mb-0">
                        ...
                        <div class="p-3 bg-light rounded-3 text-muted border-start border-4 border-primary">
                            {{ $movement->notes ?: 'No additional notes provided for this movement.' }}
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card border-0 shadow-sm rounded-4">
                <div class="card-header bg-white py-3 border-0">
                    <h6 class="fw-bold mb-0 text-danger">
                        <i class="fas fa-map-marker-alt me-2"></i>{{ $movement->type === 'in' ? 'Origin (Source)' : 'Destination (Recipient)' }}
                    </h6>
                </div>
                    ...
                    <div class="row g-4">
                        <div class="col-md-6">
                            <div class="p-3 border rounded-4 bg-white h-100">
                                ...
                                <div class="fw-bold text-dark fs-5">{{ $movement->type === 'in' ? ($movement->source_name ?: '-') : ($movement->recipient_name ?: '-') }}</div>
                                <span class="badge bg-light text-secondary border mt-1">{{ $movement->type === 'in' ? $movement->source_type : $movement->recipient_type }}</span>
                            </div>
                        </div>
                        @if($movement->type === 'out' || ($movement->type === 'in' && $movement->source_identifier))
                        <div class="col-md-6">
                            <div class="p-3 border rounded-4 bg-white h-100">
                                <small class="text-muted d-block mb-1">Identifier (Phone/ID)</small>
                                <div class="fw-bold text-dark fs-5">{{ $movement->type === 'in' ? $movement->source_identifier : $movement->recipient_identifier }}</div>
                            </div>
                        </div>
                        @endif
                        ...
                        <div class="col-md-12">
                            <div class="p-3 border rounded-4 bg-white">
                                ...
                                <div class="fw-bold text-dark"><i class="fas fa-truck me-2 text-muted"></i>{{ $movement->delivery_method ?: 'Direct Delivery' }}</div>
                            </div>
                        </div>
                        @endif
                    </div>
            ...
            <div class="card border-0 shadow-sm rounded-4 mb-4">
                ...
                <div class="card-body p-4 pt-0">
                    <div class="text-center mb-3">
                        ...
                        <h5 class="fw-bold text-dark mb-1">{{ $movement->handler_name }}</h5>
                        <p class="text-muted small mb-0">{{ $movement->handler_type }}</p>
                    </div>
                    <ul class="list-group list-group-flush small">
                        <li class="list-group-item px-0 d-flex justify-content-between">
                            <span class="text-muted">ID/Phone:</span>
                            <span class="fw-bold text-dark">{{ $movement->handler_identifier ?: 'N/A' }}</span>
                        </li>
                    </ul>
                </div>
            </div>
            ...
            <div class="card border-0 shadow-sm rounded-4">
                ...
                <div class="card-body p-4 pt-0">
                    <div class="d-flex align-items-center p-3 bg-light rounded-3">
                        ...
                        <div>
                            <div class="fw-bold text-dark mb-0">{{ $movement->gatekeeper->name }}</div>
