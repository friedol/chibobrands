            ...
            <div class="hero-slides-container">
                @foreach($heroSlides as $index => $slide)
                    <div class="hero-slide {{ $index === 0 ? 'active' : '' }}" 
                        @if($slide->image_path && !$slide->isVideo())
                            style="background-image: url('{{ asset('storage/' . $slide->image_path) }}');"
                        @endif>
                        @if($slide->image_path && $slide->isVideo())
                            <video autoplay muted loop playsinline class="hero-video-bg">
                                <source src="{{ asset('storage/' . $slide->image_path) }}" type="video/{{ strtolower(pathinfo($slide->image_path, PATHINFO_EXTENSION) == 'mov' ? 'mp4' : pathinfo($slide->image_path, PATHINFO_EXTENSION)) }}">
                            </video>
                        @endif
                            ...
                            <div class="container">
                                <div class="row justify-content-center">
                                    <div class="col-lg-10 text-center">
                                        <h1 class="hero-slide-title mb-3">{{ $slide->title }}</h1>
                                        @if($slide->subtitle)
                                            <p class="hero-slide-subtitle mb-4">{{ $slide->subtitle }}</p>
                                        @endif
                                        @if($slide->button_text && $slide->button_url)
                                            <a href="{{ $slide->button_url }}" 
                                                class="btn hero-slide-btn" 
                                                style="background-color: {{ $slide->button_color ?? '#dc3545' }};">
                                                {{ $slide->button_text }}
                                            </a>
                                        @endif
                                    </div>
            ...
            <div class="row align-items-center">
                <div class="col-lg-6 text-center text-lg-start">
                    <div class="hero-badge mb-3">
                        <i class="fas fa-star me-2"></i>{{ $channel === 'wholesale' ? 'B2B Solutions' : 'Premium Quality' }}
                    </div>
...
@endif
    ...
    <!-- Top Banner Ads -->
    @if($bannerAds->where('ad_position', 'top')->count() > 0)
        <div class="banner-ads-top mb-4" data-aos="fade-down">
            @foreach($bannerAds->where('ad_position', 'top') as $ad)
    ...
    {{-- Search controls removed from landing page.
         Use the app-bar search instead. --}}
    ...
    <div class="container">
        <div class="products-header mb-4" data-aos="fade-up">
            <div class="d-flex justify-content-between align-items-center mb-1">
                <h2 class="section-title mb-0">{{ $channel === 'wholesale' ? 'wholesale products' : 'Our Products' }}</h2>
                ...
                <div class="sticky-sidebar">
                    ...
                    <div class="card mb-4 border-0 shadow-sm overflow-hidden">
                        ...
                        <div class="list-group list-group-flush">
                            <a href="{{ request()->fullUrlWithQuery(['category' => null]) }}" class="list-group-item list-group-item-action {{ !request('category') ? 'active bg-danger border-danger' : '' }} d-flex justify-content-between align-items-center">
                                All Categories
                                <span class="badge rounded-pill bg-light text-dark">All</span>
                            </a>
                            @foreach($allCategories as $cat)
                                <a href="{{ request()->fullUrlWithQuery(['category' => $cat->name]) }}" class="list-group-item list-group-item-action {{ request('category') == $cat->name ? 'active bg-danger border-danger' : '' }} d-flex justify-content-between align-items-center">
                                    {{ $cat->name }}
                                    <span class="badge rounded-pill {{ request('category') == $cat->name ? 'bg-white text-danger' : 'bg-light text-muted' }}">
                                        {{ $cat->products_count ?? '' }}
                                    </span>
                                </a>
                            @endforeach
                ...
                <div class="d-flex justify-content-between align-items-center mb-1">
                    <h3 class="product-title">{{ $displayName }}</h3>
                                @if (!empty($product->category))
                                    <span
                                        class="category-pill">{{ is_object($product->category) ? $product->category->name ?? '' : $product->category }}</span>
                                        @endif
                                    </div>
                                            ...
                                            @if (method_exists($product, 'variantCategories') && $product->variantCategories && $product->variantCategories->count())
                                                ...
                                                @endphp
                                                                <button type="button" class="variant-option-btn"
                                                                    ...
                                                                    data-name="{{ $item->name }}"
                                                                    data-price="{{ (float) $variantPrice }}">
                                                                    <span class="variant-name">{{ $item->name }}</span>
                                                                    @if ($variantPrice > 0)
                                                                        <span
                                                                            class="variant-price">+{{ number_format($variantPrice, 0) }}</span>
                                                    @endif
                                                </button>
                                            @endforeach
                                        </div>
                            ...
                            @endif
                                            ...
                                            <div class="mt-3">
                                                ...
                                                <div class="row g-2 mb-3">
                                                    <div class="col-6">
                                                        <label for="qty-{{ $product->id }}" class="form-label mb-1" style="font-size: 0.8rem;">Quantity</label>
                                                        <div class="input-group">
                                                            <button class="btn btn-outline-secondary" type="button" onclick="decreaseQty('{{ $product->id }}')" style="font-size: 1rem; padding: 0.5rem 0.75rem; font-weight: 600;">-</button>
                                                            @php
                                                                $minOrderQty = $product->min_quantity ?? 1;
                                                            @endphp
                                                            <input id="qty-{{ $product->id }}" type="number" class="form-control text-center qty-input" 
                                                                   value="{{ $minOrderQty }}" min="{{ $minOrderQty }}" data-product-id="{{ $product->id }}" 
                                                                   style="font-size: 1.2rem; padding: 0.5rem 0.3rem; font-weight: 700;">
                                                            <button class="btn btn-outline-secondary" type="button" onclick="increaseQty('{{ $product->id }}')" style="font-size: 1rem; padding: 0.5rem 0.75rem; font-weight: 600;">+</button>
                                </div>
                            </div>
                                                    <div class="col-6">
                                                        <div class="text-end">
                                                            <div class="small text-muted" style="font-size: 0.7rem;">Unit Price</div>
                                                            <div class="fw-bold" id="unit-{{ $product->id }}" style="font-size: 0.9rem;">0 TZS</div>
                        </div>
                    </div>
                </div>
                                                ...
                                                <div class="row g-2 mb-3">
                                                    <div class="col-6">
                                                        ...
                                                        <div class="fw-bold text-primary" id="total-{{ $product->id }}" style="font-size: 1.1rem;">0 TZS</div>
                                                    </div>
                                                    <div class="col-6">
...
@empty
                ...
                <div class="col-lg-4" data-aos="fade-up" data-aos-delay="100">
                    <div class="cta-card cta-wholesale">
                        <div class="cta-content">
                            ...
                            <a href="{{ url('/b2b') }}" class="btn cta-btn">Enter Wholesale</a>
                        </div>
                    </div>
                </div>
                <!-- CTA Card 2 -->
                <div class="col-lg-4" data-aos="fade-up" data-aos-delay="200">
                    <div class="cta-card cta-printing">
                        <div class="cta-content">
                            ...
                            <a href="{{ url('/services') }}" class="btn cta-btn">Explore Services</a>
                        </div>
                    </div>
                </div>
                <!-- CTA Card 3 -->
                <div class="col-lg-4" data-aos="fade-up" data-aos-delay="300">
                    <div class="cta-card cta-support">
                        <div class="cta-content">
                            ...
                            <a href="https://wa.me/{{ $dynamicWhatsapp ?? '255655392319' }}" class="btn cta-btn">Chat with Us</a>
                        </div>
                    </div>
                </div>
            </div>
    ...
    @if($bannerAds->where('ad_position', 'bottom')->count() > 0)
        <div class="container">
                        ...
                        <div class="banner-slider-container">
                            <div class="banner-slider-wrapper">
                                <div class="banner-slider" id="bannerSlider">
                                    @foreach($bannerAds->where('ad_position', 'bottom') as $ad)
                                        <div class="banner-slide" data-ad-id="{{ $ad->id }}" data-ad-type="banner">
                                            <div class="ad-container {{ $ad->image_path ? 'has-image' : 'no-image' }}">
                                                <div class="ad-content">
                                                    @if($ad->image_path)
                                                        <img src="{{ asset('storage/' . $ad->image_path) }}" 
                                                             alt="{{ $ad->title }}" 
                                                             ...
                                                             loading="lazy">
                                                    @endif
                                                    <div class="ad-overlay {{ $ad->image_path ? 'overlay-text' : 'standalone-text' }}">
                                                        @if($ad->title)
                                                            <h5 class="ad-title">{{ $ad->title }}</h5>
                                                        @endif
                                                        @if($ad->subtitle)
                                ...
                                @if($bannerAds->where('ad_position', 'bottom')->count() > 4)
    ...
    @if($popupAds->count() > 0)
        @foreach($popupAds as $ad)
            <div class="popup-ad-modal position-{{ $ad->ad_position ?? 'center' }}" id="popupAd{{ $ad->id }}" data-ad-id="{{ $ad->id }}" data-ad-type="popup" data-duration="{{ $ad->ad_duration }}">
