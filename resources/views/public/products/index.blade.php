...
<div class="container">
    ...
    <style>
        ...
        .banner-ads-bottom {
            padding: 0 15px;
            /* Add horizontal padding instead of card */
        }
        ...
        .banner-slide {
            flex: 0 0 30%;
            /* 3 slides per view for wider ads */
            min-width: 0;
        }
    ...
    <div class="row g-4" id="products-grid">
        ...
        @if(!request()->is('/') || request()->has('category'))
        <div class="col-12 col-lg-2">
            <div class="sticky-sidebar">
                <!-- Categories Filter -->
                <div class="card mb-4 border shadow-sm overflow-hidden">
                    <div class="card-header bg-white text-dark py-3 border-bottom">
                        <h5 class="card-title mb-0 fw-bold" style="font-size: 1rem;"><i class="fas fa-list me-2"></i>Categories
                        </h5>
                    </div>
                    <div class="list-group list-group-flush">
                        <a href="{{ request()->fullUrlWithQuery(['category' => null]) }}"
                            class="list-group-item list-group-item-action {{ !request('category') ? 'active bg-white text-danger fw-bold border-start border-danger border-3' : '' }} d-flex justify-content-between align-items-center">
                            All Categories
                            <span class="badge rounded-pill bg-light text-dark">All</span>
                        </a>
                        @foreach($allCategories as $cat)
                        <a href="{{ request()->fullUrlWithQuery(['category' => $cat->name]) }}"
                            class="list-group-item list-group-item-action {{ request('category') == $cat->name ? 'active bg-white text-danger fw-bold border-start border-danger border-3' : '' }} d-flex justify-content-between align-items-center">
                            {{ $cat->name }}
                            <span
                                class="badge rounded-pill {{ request('category') == $cat->name ? 'bg-danger text-white' : 'bg-light text-muted' }}">
                                {{ $cat->products_count ?? '' }}
                            </span>
                        </a>
                        @endforeach
                ...
                <!-- Sidebar Ads -->
                @if($sidebarAds->where('ad_position', 'left')->count() > 0)
                <div class="sidebar-ads-left">
                    @foreach($sidebarAds->where('ad_position', 'left') as $ad)
                    <div class="ad-container mb-3" data-ad-id="{{ $ad->id }}" data-ad-type="sidebar">
                        <div class="ad-content">
                            ...
                            <div class="ad-overlay">
                                ...
                                @endif
                                @if($ad->subtitle)
                                <p class="ad-subtitle small">{{ $ad->subtitle }}</p>
                                @endif
                                @if($ad->button_text && $ad->button_url)
                                <a href="{{ $ad->button_url }}" class="btn btn-sm btn-primary ad-button"
                                    style="background-color: {{ $ad->button_color }}; border-color: {{ $ad->button_color }};"
        ...
        <!-- Products Column -->
        <div class="col-12 {{ (!request()->is('/') || request()->has('category')) ? 'col-lg-10' : 'col-lg-12' }}">
            <div class="row g-2 row-cols-2 row-cols-sm-3 row-cols-md-4 row-cols-lg-4">
                @forelse($products as $index => $product)
                <!-- Inline Ads - Show after every 8 products -->
                @if($index > 0 && $index % 8 === 0 && $inlineAds->count() > 0)
                <div class="col-12 mb-4">
                    <div class="inline-ads">
                        @foreach($inlineAds->take(1) as $ad)
                        <div class="ad-container" data-ad-id="{{ $ad->id }}" data-ad-type="inline">
                            <div class="ad-content text-center">
                                ...
                                <div class="ad-overlay mt-3">
                                    ...
                                    @endif
                                    @if($ad->button_text && $ad->button_url)
                                    <a href="{{ $ad->button_url }}" class="btn btn-primary ad-button"
                                        style="background-color: {{ $ad->button_color }}; border-color: {{ $ad->button_color }};"
                                        onclick="trackAdClick({{ $ad->id }})">
                                        {{ $ad->button_text }}
                                    </a>
                ...
                <div class="col" data-aos="fade-up" data-aos-delay="{{ ($index % 6) * 100 }}">
                    @php
                    ...
                    @endphp
                    <div class="product-card h-100" onclick="navigateToProduct('{{ $viewUrl }}')"
                        style="cursor: pointer;">
                        <div class="product-image-wrapper">
                            @if ($product->images->count() > 0)
                            <img src="/storage/{{ ltrim($product->images->first()->image_path, '/') }}"
                                class="img-cover" alt="{{ $displayName }}">
                            @else
                        ...
                        <div class="p-3">
                            <div class="toolbar-top" onclick="event.stopPropagation();">
                                <button class="btn btn-add btn-sm text-white" data-bs-toggle="modal"
                                    data-bs-target="#addToCartModal-{{ $product->id }}"><i
                                        class="fas fa-cart-plus me-1"></i>Place Order</button>
                                <a href="{{ $viewUrl }}" class="btn btn-outline-secondary btn-sm">Details</a>
                            </div>
                            <div class="d-flex justify-content-between align-items-center mb-1">
                                <h3 class="product-title">{{ $displayName }}</h3>
                                @if (!empty($product->category))
                                <span class="category-pill">{{ is_object($product->category) ? $product->category->name
                            ...
                            <div class="mb-2">
                                ...
                                <div class="text-center">
                                    ...
                                    <div class="price text-success fw-bold">{{ number_format((float) $displayPrice) }}
                                        TZS</div>
                                    <div class="small text-muted">
                                        <del class="text-muted">{{ number_format((float) $base) }} TZS</del>
                                    ...
                                    <div class="small text-success">
                                        ...
                                        @else
                                        {{ number_format($offer->discount_value) }} TZS OFF
                                        @endif
                                    </div>
                                </div>
                                @else
                                <span class="price">{{ number_format((float) $displayPrice) }} TZS</span>
                                @endif
                            </div>
                            <div class="product-meta">{{ $product->barcode }}</div>
                        </div>
                    </div>
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <div class="row g-3">
                                        <div class="col-md-4">
                                            @if ($product->images->count() > 0)
                                            <img id="modalImage-{{ $product->id }}"
                                                src="/storage/{{ ltrim($product->images->first()->image_path, '/') }}"
                                                data-base-image="/storage/{{ ltrim($product->images->first()->image_path, '/') }}"
                                                class="img-fluid rounded" alt="{{ $displayName }}"
                                        ...
                                        <div class="col-md-8">
                                            <h6 class="mb-2">Price
                                                ({{ $channel === 'wholesale' ? 'Wholesale' : 'Retail' }})</h6>
                                            @php
                                            /* Use the already calculated price with discounts */
                                            $modalPrice = $displayPrice;
                                            @endphp
                                            <div class="alert alert-success mb-3">
                                                <h4 class="mb-0">{{ number_format($modalPrice, 0) }} TZS per unit</h4>
                                                @if ($hasDiscount)
                                                <small class="text-muted">
                                                    <del>{{ number_format($base, 0) }} TZS</del>

                                                </small>
                                                @endif
                                            </div>

                                            @if (method_exists($product, 'variantCategories') &&
                                            $product->variantCategories && $product->variantCategories->count())
                                            <h6 class="mt-3 mb-3 fw-bold">Variants</h6>
                                            @foreach ($product->variantCategories as $cat)
                                            <div class="variant-section mb-3">
                                                <label class="variant-label">{{ $cat->category }}</label>
                                                <div class="variant-options">
                                                    ...
                                                    <button type="button" class="variant-option-btn"
                                                        ...
                                                        data-name="{{ $item->name }}"
                                                        data-price="{{ (float) $variantPrice }}" @php
                                                        $matchingImage=$product->images->filter(function($img) use
                                                        ($item) {
                                                        return $img->color && strtolower(trim($img->color)) ===
                                                        strtolower(trim($item->name));
                                                        })->first();
                                                        @endphp
                                                        @if($matchingImage)
                                                        data-image-url="/storage/{{ ltrim($matchingImage->image_path,
                                                        '/') }}"
                                                        @endif
                                                        >
                                                        <span class="variant-name">{{ $item->name }}</span>
                                                        @if ($variantPrice > 0)
                                                        <span class="variant-price">+{{ number_format($variantPrice, 0)
                                                            }}</span>
                                                        @endif
                                                    </button>
                                                    @endforeach
                                            ...
                                            <div class="mt-3">
                                                ...
                                                <div class="row g-2 mb-3">
                                                    <div class="col-6">
                                                        <label for="qty-{{ $product->id }}" class="form-label mb-1"
                                                            style="font-size: 0.8rem;">Quantity</label>
                                                        <div class="input-group">
                                                            <button class="btn btn-outline-secondary" type="button"
                                                                onclick="decreaseQty('{{ $product->id }}')"
                                                                style="font-size: 1rem; padding: 0.5rem 0.75rem; font-weight: 600;">-</button>
                                                            @php
                                                            $minOrderQty = $product->min_quantity ?? 1;
                                                            @endphp
                                                            <input id="qty-{{ $product->id }}" type="number"
                                                                class="form-control text-center qty-input"
                                                                value="{{ $minOrderQty }}" min="{{ $minOrderQty }}"
                                                                data-product-id="{{ $product->id }}"
                                                    ...
                                                    <div class="col-6">
                                                        <div class="text-end">
                                                            <div class="small text-muted" style="font-size: 0.7rem;">
                                                                Unit Price</div>
                                                            <div class="fw-bold" id="unit-{{ $product->id }}"
                                                                style="font-size: 0.9rem;">0 TZS</div>
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="row g-2 mb-3">
                                                    <div class="col-6">
                                                        <div class="small text-muted" style="font-size: 0.7rem;">Total
                                                            Amount</div>
                                                        <div class="fw-bold text-primary" id="total-{{ $product->id }}"
                                                            style="font-size: 1.1rem;">0 TZS</div>
                                                    </div>
                                                    <div class="col-6">
                                                        <button class="btn btn-danger w-100 add-to-cart-confirm"
                                                            ...
                                                            data-base="{{ (float) ($channel === 'wholesale' ? $product->b2b_base_price ?? ($product->buying_price ?? 0) : $product->retail_base_price ?? ($product->buying_price ?? 0)) }}"
                                                            data-minqty="{{ (int) ($product->min_quantity ?? 1) }}"
                ...
                <div class="col-12">
                    <div class="empty-state text-center py-5">
                        ...
                        <a href="{{ $channel === 'wholesale' ? route('wholesale.products') : route('products.index') }}"
        ...
        <!-- Right Sidebar Ads -->
        @if($sidebarAds->where('ad_position', 'right')->count() > 0)
            ...
            <div class="sidebar-ads-right mt-4">
                <div class="card border-0 shadow-sm overflow-hidden">
                    ...
                    <div class="card-body p-2">
                        @foreach($sidebarAds->where('ad_position', 'right') as $ad)
                        <div class="ad-container mb-3" data-ad-id="{{ $ad->id }}" data-ad-type="sidebar">
                            <div class="ad-content">
                                @if($ad->image_path)
                                <img src="{{ asset('storage/' . $ad->image_path) }}" alt="{{ $ad->title }}"
...
<section class="py-5 mt-5">
    <div class="container">
        <div class="text-center mb-5" data-aos="fade-up">
            <h2 class="section-title">Why Choose CHIBO BRAND?</h2>
            <div class="section-divider mx-auto mb-4"
                style="width: 80px; height: 4px; background: #dc3545; border-radius: 2px;"></div>
            <p class="text-muted mx-auto" style="max-width: 600px;">Experience the perfect blend of quality, speed, and
                service with Africa's leading printing and branding partner.</p>
        </div>
        ...
        <div class="row g-4">
            <div class="col-lg-3 col-md-6" data-aos="fade-up" data-aos-delay="100">
                <div class="feature-card h-100 p-4 text-center">
                    ...
                    <p class="text-muted mb-0">Premium materials and state-of-the-art equipment ensure the highest
                        quality output for every single project.</p>
                </div>
            </div>
            ...
            <div class="col-lg-3 col-md-6" data-aos="fade-up" data-aos-delay="200">
                <div class="feature-card h-100 p-4 text-center">
                    ...
                    <p class="text-muted mb-0">Quick turnaround times without compromise. Most orders completed and
            ...
            <div class="col-lg-3 col-md-6" data-aos="fade-up" data-aos-delay="300">
                <div class="feature-card h-100 p-4 text-center">
                    ...
                    <h5 class="fw-bold mb-3">Competitive Pricing</h5>
                    <p class="text-muted mb-0">Affordable pricing structures that fit your budget while maintaining the
            ...
            <div class="col-lg-3 col-md-6" data-aos="fade-up" data-aos-delay="400">
                <div class="feature-card h-100 p-4 text-center">
                    ...
                    <p class="text-muted mb-0">Our dedicated customer support team is available 24/7 to assist you with
                        any questions or custom requirements.</p>
                </div>
            </div>
        </div>
    </div>
...
<div class="popup-ad-modal position-{{ $ad->ad_position ?? 'center' }}" id="popupAd{{ $ad->id }}"
    data-ad-id="{{ $ad->id }}" data-ad-type="popup" data-duration="{{ $ad->ad_duration }}">
    <div class="popup-ad-content">
        @if($ad->ad_closable)
        <button class="popup-ad-close" onclick="closePopupAd({{ $ad->id }})">
            <i class="fas fa-times"></i>
        </button>
        @endif
        <div class="popup-ad-body {{ $ad->image_path ? 'has-image' : 'no-image' }}">
            @if($ad->image_path)
            <img src="{{ asset('storage/' . $ad->image_path) }}" alt="{{ $ad->title }}" class="img-fluid w-100 ad-image"
                style="border-radius: 8px;">
            @endif
            @if($ad->title || $ad->subtitle || ($ad->button_text && $ad->button_url))
            <div class="popup-ad-text {{ $ad->image_path ? 'overlay-text' : 'standalone-text' }}">
                @if($ad->title)
...
<div class="mt-4 d-none">{{ $products->links() }}</div>
@endif
...
<style>
    ...
    .hero-slideshow-search-overlay .search-icon {
        left: 18px;
        font-size: 16px;
    }
...
<script>
    ...
    async function fetchUnitPriceForQuantity(barcode, customerType, quantity, fallbackBase) {
        ...
        const customer = customerType || '{{ $channel }}';
    ...
    async function updatePriceDisplay(pid) {
        ...
        const channel = confirmBtn.dataset.channel || '{{ $channel }}';
    ...
    document.addEventListener('click', function (e) {
        if (e.target.classList.contains('variant-option-btn')) {
            ...
            if (btn.classList.contains('active')) {
                ...
                // Switch image if variant has a specific image URL
                const modalImg = document.getElementById(`modalImage-${pid}`);
                if (modalImg) {
                    if (btn.dataset.imageUrl) {
                        modalImg.src = btn.dataset.imageUrl;
                    } else if (cat.toLowerCase().includes('color') || cat.toLowerCase().includes('rangi')) {
                        modalImg.src = modalImg.dataset.baseImage;
                    }
                }
            }
