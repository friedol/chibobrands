...
<head>
    <meta charset="utf-8">
    <title>Balance Sheet – {{ \Carbon\Carbon::parse($dateFrom)->format('d M') }} – {{ $asAt->format('d M Y') }}</title>
    <style>
...
<body>
    <div class="header">
        ...
        <div class="header-right">
            ...
            <p class="as-at">{{ \Carbon\Carbon::parse($dateFrom)->format('d M') }} – {{ $asAt->format('d M Y') }}</p>
            <p class="sub">{{ $currentDept ? $currentDept->name : 'Consolidated' }} · Generated: {{ now()->format('M d, Y H:i') }}</p>
    ...
    <table>
        ...
        <tbody>
            <tr>
                <td>Mobile (net)</td>
                <td class="num">{{ number_format($cash_mobile ?? 0) }}</td>
                <td>Liabilities</td>
                <td class="num">{{ number_format($liabilities) }}</td>
            </tr>
            <tr>
                <td>Cash (net)</td>
                <td class="num">{{ number_format($cash_cash ?? 0) }}</td>
                <td>Equity (net position)</td>
                <td class="num">{{ number_format($equity) }}</td>
            </tr>
            <tr>
                <td>Bank (net)</td>
                <td class="num">{{ number_format($cash_bank ?? 0) }}</td>
                <td></td>
                <td class="num"></td>
            </tr>
            <tr>
                <td>Accounts receivable</td>
                <td class="num">{{ number_format($receivables) }}</td>
                <td></td>
                <td class="num"></td>
            </tr>
            <tr class="total">
                <td>Total assets</td>
                <td class="num">{{ number_format($total_assets) }}</td>
                <td>Total liabilities & equity</td>
                <td class="num">{{ number_format($total_liabilities_equity) }}</td>
            </tr>
        </tbody>
    </table>

    @if(!$currentDept && $department_breakdown->isNotEmpty())
    <div class="section-title">By department ({{ \Carbon\Carbon::parse($dateFrom)->format('d M') }} – {{ $asAt->format('d M Y') }})</div>
    <table class="dept-table">
        ...
        <tbody>
            ...
            <tr>
                <td>{{ $d['name'] }}</td>
                <td class="num">{{ number_format($d['mobile'] ?? 0) }}</td>
                <td class="num">{{ number_format($d['cash'] ?? 0) }}</td>
                <td class="num">{{ number_format($d['bank'] ?? 0) }}</td>
                <td class="num">{{ number_format($d['receivables']) }}</td>
                <td class="num">{{ number_format($d['total_assets']) }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
