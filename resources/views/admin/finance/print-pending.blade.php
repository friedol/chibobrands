        ...
        <div class="meta-container">
            ...
            <div class="text-end">
                ...
                <div class="small">
                    ...
                    @endphp
                    <strong>Total Outstanding:</strong> TZS {{ number_format($totalBalance) }}<br>
                    <strong>Design Tasks:</strong> {{ $pendingTasks->count() }} (TZS {{ number_format($pendingTasks->sum('balance')) }})<br>
                    <strong>Product Orders:</strong> {{ $pendingOrders->count() }} (TZS {{ number_format($pendingOrders->sum('balance')) }})
                </div>
            </div>
        </div>
                ...
                <table class="table-pro">
                    ...
                    <tbody>
                        ...
                        <tr style="background-color: #f8f8f8; border-top: 2px solid #222;">
                            ...
                            <td class="text-end fw-bold text-success" style="padding: 10px;">{{ number_format($pendingTasks->sum('amount_paid')) }}</td>
                            <td class="text-end fw-bold text-danger" style="padding: 10px;">{{ number_format($pendingTasks->sum('balance')) }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
                    ...
                    <tbody>
                        @foreach($pendingOrders as $order)
                            <tr>
                                ...
                                <td>
                                    <div class="fw-bold">{{ $order->user->name ?? 'Walk-in' }}</div>
                                    <div class="text-muted small">{{ $order->user->phone ?? '' }}</div>
                                </td>
                                <td>{{ $order->created_at->format('d/m/Y') }}</td>
                                <td class="text-end">{{ number_format($order->total_amount) }}</td>
                                <td class="text-end text-success">{{ number_format($order->amount_paid) }}</td>
                                <td class="text-end fw-bold text-danger">{{ number_format($order->balance) }}</td>
                            </tr>
                        @endforeach
                        <tr style="background-color: #f8f8f8; border-top: 2px solid #222;">
                            <td colspan="3" class="text-end fw-bold" style="padding: 10px;">TOTALS:</td>
                            <td class="text-end fw-bold" style="padding: 10px;">{{ number_format($pendingOrders->sum('total_amount')) }}</td>
                            <td class="text-end fw-bold text-success" style="padding: 10px;">{{ number_format($pendingOrders->sum('amount_paid')) }}</td>
                            <td class="text-end fw-bold text-danger" style="padding: 10px;">{{ number_format($pendingOrders->sum('balance')) }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
