    ...
    <div class="collapse {{ request()->anyFilled(['period', 'start_date', 'end_date', 'search']) ? 'show' : '' }} mb-4"
        ...
        <div class="card border-0 shadow-sm border-top border-4 border-primary">
            <div class="card-body bg-light p-3">
                <form action="{{ route('admin.finance.departments.index') }}" method="GET" class="row g-2 align-items-end"
                    ...
                    <div class="col-12 col-md-3">
                        ...
                        <div class="input-group input-group-sm">
                            ...
                            <input type="text" name="search" class="form-control" placeholder="Dept name..." value="{{ request('search') }}">
                        </div>
                    </div>
                    ...
                    <div class="col-12 col-md-3">
                        ...
                        <select name="period" id="periodSelect" class="form-select form-select-sm">
                            <option value="all" {{ ($period ?? '') == 'all' ? 'selected' : '' }}>All Time (History)</option>
                    ...
                    <div class="col-12 col-md-auto ms-auto">
                        <div class="btn-group shadow-sm w-100">
                            ...
                            <a href="{{ route('admin.finance.departments.index') }}" class="btn btn-dark btn-sm px-4 fw-bold">RESET</a>
                        </div>
                    </div>
                </form>
            </div>
                ...
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ Str::limit($dept->description, 50) ?: 'No description' }}</td>
                            <td class="text-center"><span class="badge bg-info bg-opacity-10 text-info px-3">{{ $dept->design_tasks_count }}</span></td>
                            <td class="text-center"><span class="badge bg-success bg-opacity-10 text-success px-3">{{ $dept->orders_count }}</span></td>
                            <td class="text-center"><span class="badge bg-danger bg-opacity-10 text-danger px-3">{{ $dept->expenses_count }}</span></td>
                            <td class="text-end pe-4">
                                @if(auth()->user()->hasPermission('manage_finance') || auth()->user()->role === 'accountant')
                                <button class="btn btn-sm btn-outline-primary me-1" data-bs-toggle="modal" data-bs-target="#editDepartmentModal{{ $dept->id }}" data-no-global-handler>
                                    <i class="fas fa-edit"></i>
                                </button>
                                <form action="{{ route('admin.finance.departments.destroy', $dept->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Are you sure you want to delete this department?')">
                                    @csrf @method('DELETE')
                                    <button class="btn btn-sm btn-outline-danger" {{ ($dept->design_tasks_count > 0 || $dept->orders_count > 0) ? 'disabled' : '' }} data-no-global-handler>
...
<div class="modal fade" id="addDepartmentModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <form action="{{ route('admin.finance.departments.store') }}" method="POST">
            @csrf
...
<div class="modal fade" id="editDepartmentModal{{ $dept->id }}" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <form action="{{ route('admin.finance.departments.update', $dept->id) }}" method="POST">
            @csrf @method('PUT')
            <div class="modal-content">
                <div class="modal-header">
