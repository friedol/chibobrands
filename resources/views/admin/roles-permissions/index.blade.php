...
@section('page-title', 'Security Permissions Configuration')
...
<div class="container-fluid py-4">
    ...
    <div class="row mb-4 align-items-center">
        ...
        <div class="col-sm-6 text-sm-end">
            <a href="{{ route('admin.admins.index') }}" class="btn btn-outline-primary btn-sm rounded-pill px-4 shadow-sm mt-3 mt-sm-0">
        ...
        <div class="col-12 col-xl-6">
            <div class="role-card">
                <div class="role-card-header">
                    <div class="role-icon-box {{ $colors['class'] }}">
                        <i class="fas {{ $colors['icon'] }}"></i>
                    </div>
                    <div class="role-name">
                        {{ $roleData['name'] }}
                        <span class="status-badge {{ $colors['class'] }}">{{ $roleCounts[$roleKey] ?? 0 }} Users</span>
                    </div>
                    <div class="role-desc">{{ $roleData['description'] }}</div>
                </div>

                <div class="permissions-area">
                    <form method="POST" action="{{ route('admin.roles-permissions.update', $roleKey) }}">
                        @csrf
                        @method('PUT')
                        ...
                        @foreach($categorized as $catName => $perms)
                            @if(count($perms) > 0)
                            <div class="perm-group">
                                <div class="perm-group-header">{{ $catName }}</div>
                                @foreach($perms as $permission => $hasPermission)
                                <div class="perm-row">
                                    <div class="perm-info">
                                        <div class="perm-dot" style="background: {{ $hasPermission ? '#0d6efd' : '#cbd5e0' }}"></div>
                                        <span class="perm-name">{{ ucwords(str_replace('_', ' ', $permission)) }}</span>
                                    </div>
                        ...
                        <div class="card-actions d-flex align-items-center justify-content-between">
                            @if($roleKey !== 'super_admin')
                                <span class="text-muted x-small fw-bold">{{ strtoupper($roleKey) }} PROTOCOL</span>
